...
- 三角色：Planner / Worker / Reviewer
- 流程：规划 → 按依赖分配 Worker → Reviewer 审查 → 未通过重试(最多 2 次)
- SubAgent IOException 返回 ERROR 类型
- 所有子代理共享 ToolRegistry 和 MemoryManager
- WorkerPool 默认预热 2 个 Worker，同一 DAG 批次无空闲实例时按需扩容至 4 个；批次完成后关闭多余 Worker 并缩容回 2 个
- Worker 通过幂等 Lease 独占租借，归还前 `clearHistory()`；池关闭期间仍在执行的 Worker 在 Lease 归还时关闭，不重新入队
- `SubAgent` / `AgentOrchestrator` 实现 `AutoCloseable`，CLI/TUI 的 Team 入口使用 try-with-resources 覆盖正常、异常和取消路径
- 每个并行步骤即时创建独立 Reviewer 并在步骤结束时关闭；角色实例分别持有独立 conversationHistory
- 生命周期所有权只覆盖 SubAgent 自身状态，不关闭共享的 LlmClient、ToolRegistry、MemoryManager、MCP 或 SnapshotService
- `agent-eval` Profile 保留为历史实验和回归工具：在隔离工作区对比三种 Agent 路径，结束后注入隐藏测试；真实调用默认不进入 quick/CI，结果见 `docs/agent-ab-evaluation-pilot-report-2026-08-10.md`。
