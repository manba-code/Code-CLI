...
> 状态：Accepted  
> 目标版本：V1  
> 实现进度：前六条产品切片已完成；第七条框架、Pilot 修复、代表性小样和同模型 36 次复跑均已完成。GLM 完整复跑未满足当时的首轮门槛；DeepSeek 代表性小样 A/B/C 均为 100%，出现成功率天花板，证明模型/API 组合影响显著，但不能据此判定 ChangeSpec 的增量质量或修复价值。评测报告已改为分维度四态结论；真人总人时协议和采集模板已经设计，可信验收、人力节省和正式统计结论仍待实际实验
...
- 13 个 fixture（4 small / 5 medium / 4 high）为需求语义声明逐项公开证据契约；其中 `clarified-display-name` 向 A/B/C 提供完全相同的统一澄清记录，用于测量明确需求契约而非信息差；配对 Draft 锁定前必须通过 command、JUnit glob、最低执行测试数和 Criterion 引用校验，报告分列公开证据执行数/任务下限；每个 fixture 的免费预检还必须证明公开 Verifier 能拒绝一个确定性单点错误突变；
...
6. ✅ 一次证据驱动修复；
7. 🟡 A/B/C 评测与指标报告：框架、首次 Pilot 修复和 GLM 同模型 36 次复跑已完成；DeepSeek 代表性小样三组均为 100%，暴露成功率天花板。报告已改为四态分维度结论、A→B/B→C/A→C 配对作用拆分、95% 区间、修复机会分母、公开证据执行/下限，以及客观正确/可信决策/失败实际/惩罚 TTA 分列；任务目录现为 13 个 fixture（4 small / 5 medium / 4 high），专门的统一澄清记录任务、逐项公开证据契约与每任务一个确定性单点突变均已落地并通过免费预检。真人总人时协议与采集模板已完成，后续仍需至少 3 次/组的新版付费基线和实际真人效率验证。
