...
- 12 个 fixture（每层 4 个）为需求语义声明逐项公开证据契约；配对 Draft 锁定前必须通过 command、JUnit glob、最低执行测试数和 Criterion 引用校验，报告分列公开证据执行数/任务下限；每个 fixture 的免费预检还必须证明公开 Verifier 能拒绝一个确定性单点错误突变；
...
6. ✅ 一次证据驱动修复；
7. 🟡 A/B/C 评测与指标报告：框架、首次 Pilot 修复和 GLM 同模型 36 次复跑已完成；DeepSeek 代表性小样三组均为 100%，暴露成功率天花板。报告已改为四态分维度结论、A→B/B→C/A→C 配对作用拆分、95% 区间、修复机会分母、公开证据执行/下限，以及客观正确/可信决策/失败实际/惩罚 TTA 分列；任务目录现为 12 个 fixture（每层 4 个），逐项公开证据契约与每任务一个确定性单点突变均已落地。后续仍需至少 3 次/组的新版付费基线、专门的歧义澄清任务和真人效率验证。
