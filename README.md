...
- `/spec <代码变更需求>` - 使用当前模型和 Project Context 生成经结构校验的 ChangeSpec Draft；Enter 确认后不可覆盖地保存到 `.paicli/specs/<specId>-r1.md`，记录 ReAct 前 workspace baseline，执行后采集本轮 changed files/final diff，并依次运行 command/JUnit 与最终 `path_scope` Verifier。系统随后逐条生成 Criterion Result；确定性条件全部通过时用 `P / F / S` 处理 Human Criterion，归约为最终 Verdict，并把紧凑 Evidence、指标和 diff 保存到 `.paicli/runs/<run-id>/`。Verifier 命令仍单独经过 HITL 与 CommandGuard
