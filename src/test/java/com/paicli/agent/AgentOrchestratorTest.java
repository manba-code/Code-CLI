...
class AgentOrchestratorTest {
    ...
    void shouldRunIndependentStepsInParallel(@TempDir Path tempDir) throws Exception {
        ...
        assertEquals(2, orchestrator.workerPoolStats().total(), "batch should trim workers to minimum");
        orchestrator.close();
    }

    @Test
    void shouldExpandToFourWorkersAndTrimAfterParallelBatch(@TempDir Path tempDir) {
        CountDownLatch workersInFlight = new CountDownLatch(4);
        AtomicInteger peakConcurrency = new AtomicInteger();
        AtomicInteger currentConcurrency = new AtomicInteger();
        ...
        Function<String, LlmClient.ChatResponse> dispatcher = body -> {
            if (body.contains("请为以下任务制定执行计划")) {
                return response("""
                        {"steps":[
                          {"id":"a","description":"并行任务A","type":"ANALYSIS","dependencies":[]},
                          {"id":"b","description":"并行任务B","type":"ANALYSIS","dependencies":[]},
                          {"id":"c","description":"并行任务C","type":"ANALYSIS","dependencies":[]},
                          {"id":"d","description":"并行任务D","type":"ANALYSIS","dependencies":[]}
                        ]}
                        """);
            }
            if (body.contains("原始任务")) {
        ...
        orchestrator.run("测试四路并行");

        assertEquals(4, peakConcurrency.get());
        assertEquals(2, orchestrator.workerPoolStats().total());
        orchestrator.close();
        assertTrue(orchestrator.workerPoolStats().closed());
        assertThrows(IllegalStateException.class, () -> orchestrator.run("再次执行"));
    }
