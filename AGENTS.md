...
- ChangeSpec V1 已完成前六条产品切片；第七条切片的 A/B/C 评测框架、六个分层任务、配对 Spec、首次候选快照、隐藏 Oracle、指标归约和报告入口已经实现。较早的 `safe-divider × 3轮` 真实 LLM smoke 中 A/B/C 成功率分别为 66.67%/66.67%/33.33%；C 有一轮在首次零改动后通过 Evidence 修复成功，另有一轮因弱 Spec 只使用 `path_scope` 而公开误放行。该弱 Spec 缺口已增加双层防护：Codec 要求每条非 scope deterministic Criterion 至少引用一个 command Verifier；评测配对 Draft 还必须引用任务允许列表中的完整命令，否则按 `DRAFT_INVALID` 保存诊断且不进入 B/C。修复后的 `safe-divider × 1轮` smoke 中 A/B/C 均通过，B/C digest 1/1 一致，C 首轮失败后一次修复通过；尚未运行完整 Pilot。配对 Draft 最终无效时会保存脱敏、截断后的两次 Draft 诊断并在报告中链接，Codec 类型错误包含具体字段路径；评测器以 `NO_CHANGE_COMPLETION` 标记完整结束、隐藏失败且零改动的 Spec Run。B/C 共用同一份锁定 Spec/digest，B 仅通过 `RunOptions` 关闭修复，C 保持 deterministic `FAIL` 后最多一次 Evidence 驱动修复；生产默认行为不变。`mvn test -Pchange-spec-eval` 默认执行 6 个任务 × 3 组 × 2 次并产生 Token 费用；自动 Pilot 的 `human_intervention_time` 必须报告为 `N/A`。
