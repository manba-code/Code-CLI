...
- ChangeSpec V1 已完成前六条产品切片、首次 Pilot 修复和同模型复跑。2026-08-23 的 `glm-4.6v-flashx` 完整 36 次复跑中 A/B/C 成功率为 58.33%/41.67%/41.67%，配对 Draft/digest 12/12，C 虚假完成率 0%，结果仍不能支持已提效。评测修复已落地：结构或语义资格失败共用 Draft Generator 的最多两次纠错链路，纠错提示明确 `path_scope`/command 的职责、双向引用和去重规则；B/C 产品耗时包含配对 Draft；报告分列产品耗时、成功 TTA、失败截断 TTA/截断数并支持成本币种；Windows Oracle 日志容错解码；Agent 停滞检测覆盖重复两步工具周期；自动评测默认限制每个 ReAct 阶段 15 轮/250k Token，生产默认行为不变；ReAct 初始/修复失败会保留真实失败原因。B/C 继续共用锁定 Spec/digest，B 只关闭修复，C 最多一次 Evidence 修复；`NO_CHANGE_COMPLETION` 和隐藏 Oracle 继续负责识别公开误放行。原模型代表性小样与完整复跑均已归档，后续跨模型对照必须冻结当前代码、任务、seed、重复次数和预算。`mvn test -Pchange-spec-eval` 会产生费用；自动 Pilot 的 `human_intervention_time` 必须为 `N/A`。详见 `docs/change-spec-pilot-remediation-checklist.md`。
