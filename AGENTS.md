...
- PaiChange Phase 1–6 本地后端、最小 Web 与离线 Mock 演示已实现：`serve --http` 共用 localhost 提供 `/v1/changes` 创建/列表/详情/事件/两阶段决策，原 `/v1/threads` 保持兼容。M5 已将 Runtime 凭据映射为服务端可信 Principal，并按仓库派生 projectId 执行四角色动作 RBAC；旧 `actorId` 只校验与认证 subject 一致，不能决定操作者。默认单 API Key 固定为 `local-user`，仅是单操作者 localhost 兼容模式，不是共享身份方案；具体 OIDC 提供方和持久化成员目录未接入。M3 已将 STANDARD/RESTRICTED/LOCKED_DOWN 和项目版本规则接入 Worker，需确认调用使用持久化精确审批并复用 M5 权限。ChangeWorkflow 集中维护状态与发布资格；Worker Job 只携带 changeId，SQLite Mock PR/Check 幂等保存。发布前核对当前锁定 Spec、Git 分支 head、控制面哈希验证后的原始 Verdict/Evidence、当前交付判断与审批；HIGH 无有效 Delivery Approval 不得 success，当前交付判断 NEEDS_HUMAN 保持 pending 且批准交付返回 422，当前交付判断失败不得 success。同源 `/changes` 提供最小 Web；`-Dpaichange.demo=true` 显式启用离线 fixture，Draft/ReAct 使用确定性替身，真实复用 worktree、SpecExecutionEngine、Verifier、一次修复和 SQLite。M6a 可显式启用有限 Docker 执行边界，但本机真实 Docker 验收未完成；真实 SCM 与 M6b 生产存储未实现。当前默认演示仍只可称本地可演示 MVP，不代表生产交付或真实模型效果。
...
- PaiChange M6a 目标主机验收（2026-09-06）：Docker Desktop 29.7.2 上以本机预置的 `alpine@sha256:14358309a308569c32bdc37e2e0e9694be33a9d99e68afb0f5ff33cc1f695dce` 运行 7 条真实容器用例，不 pull/build。实测覆盖双任务 worktree 隔离、宿主/metadata/公网默认拒绝、带 projectId/策略摘要的 internal 假代理 allowlist 与审计、错误 label/digest fail closed、cgroup 内存 OOM/PID 拒绝、命令/任务超时、主动取消、异常退出、Secret 到期、进程树清理、应用级 orphan 恢复及 Evidence 跨重启复核/篡改阻断。发现并修复三项真实缺陷：补齐小写代理环境变量、解除阻塞 `docker exec` 对并发取消的 session 锁、将 Secret tmpfs 设为配置的非 root uid/gid。M6a 针对性 36 tests、M1/M2/M3/M5 扩展回归 133 tests、Node Web 16 tests、quick 964 tests（5 skipped）均 0 failures/errors，打包成功；浏览器在应用重启后复核同一 COMPLETED 任务及 `VERIFIED` Evidence manifest。Docker Desktop daemon 重启故障注入仍待单独授权。未运行付费模型评测或真实 SCM，也不能把 Docker 称为绝对安全边界。
