...
- PaiChange M8 本地实现（2026-09-07）：新增 GitHub Settings/Client/WorkItem/SCM Adapter，复用既有 Workflow、publication ledger、RBAC、Evidence 与两阶段审批；`ConfiguredScm` 集中支持 `PAICHANGE_SCM=mock|gitlab|github` 严格单选，默认及离线 demo 保持 Mock。loopback 假 GitHub + 临时 bare remote 覆盖 Issue 幂等导入、真实 branch push、PR/status 超时或响应不明对账、完成事件落账失败重放、远端 head 不一致、closed PR、401/429 与 Token 脱敏；既有假 GitLab 同步回归。本轮 SCM/装配/启动/健康针对性 18 tests、Node Web 18 tests、M7b 容器 5+1 tests、quick 989 tests（16 skipped）和 package 均通过。已新增普通 CI、手动 M6b/M7a/M7b 容器 CI 与 Tag Release workflow。真实 GitHub/GitLab 仓库验收、托管 CI 实跑和 Release Tag 仍待专用资源与明确授权，不能宣称 M8 最终完成。详见 `docs/paichange-m8-implementation.md`。
