...
- PaiChange M8（2026-09-07）：新增 GitHub Settings/Client/WorkItem/SCM Adapter，复用既有 Workflow、publication ledger、RBAC、Evidence 与两阶段审批；`ConfiguredScm` 集中支持 `PAICHANGE_SCM=mock|gitlab|github` 严格单选，默认及离线 demo 保持 Mock。loopback 假 GitHub + 临时 bare remote 覆盖 Issue 幂等导入、真实 branch push、PR/status 超时或响应不明对账、完成事件落账失败重放、远端 head 不一致、closed PR、401/429 与 Token 脱敏；既有假 GitLab 同步回归。SCM/装配/启动/健康针对性 18 tests、Node Web 18 tests、M7b 容器 5+1 tests、quick 990 tests（16 skipped）和 package 均通过。普通 CI、手动容器矩阵与 `v16.1.1` Tag Release 均已在 GitHub Actions 实跑通过；托管容器报告为 M6b 1、M7a 1、M7b 6 tests，全部 0 failure/error/skip；下载 JAR 的 SHA-256 与无凭据 `--help` smoke 已独立核验。真实 GitHub Issue #1 → 精确任务分支 → PR #3 → head-bound Status 已验收：Issue 导入/PR/Status/publication ledger 均为 1，完成事件响应不明后保持 `PUBLISHING`，清除本地 publication 回执并重启仍只对账既有远端对象后进入 `COMPLETED`；无效 Token、跨项目主体和 SERVICE 审批均 fail closed。诊断尝试 PR #2 是不同 ChangeTask/publication，保留 open 且未合并。真实 GitLab 仍待单独授权，不能宣称 M8 全部完成。详见 `docs/paichange-m8-implementation.md`。
...
- 下一步：按 `docs/paichange-m8-resume-release-plan.md` 在获得专用 GitLab project 与明确授权后完成真实 GitLab 闭环；OAuth / sampling / recovery 保留为后续 MCP 增强，不阻塞本地实现
