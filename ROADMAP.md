...
GitLab Adapter 已交付但真实实例验收已暂缓：当前无法提供专用测试 project，待后续单独提供测试 project、Issue、最小权限 Token 和真实写入授权后恢复；这不属于 M4 代码验收失败。简历发布版本的下一段必要工作限定为 GitHub Adapter、GitHub/GitLab 真实测试仓库闭环、公开 CI 与版本发布，见 [M8 收口计划](docs/paichange-m8-resume-release-plan.md)。Jira、Webhook、多 SCM 同时启用、真实 IdP、登录页/Refresh Token、SCIM/组织同步仍不属于该收口版本。M6a Docker 只形成有限的单机任务执行边界；M6b/M7a/M7b 也只形成生产存储、身份与运维的最小切片，不等于高可用、多区域、Kubernetes 或完整生产部署。启动、验收和边界见 README 的 PaiChange 部分及 M3–M7b 实施记录。
