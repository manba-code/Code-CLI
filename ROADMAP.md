...
M8 本地实现已交付 GitHub Issue/branch/PR/status Adapter、`mock|gitlab|github` 单选装配、loopback 假 GitHub + bare remote 闭环和三类 GitHub Actions；Workflow、Worker、API、RBAC、Evidence 与 publication ledger 没有按 provider 分叉。GitHub/GitLab 真实实例验收仍暂缓：待单独提供专用仓库、Issue、最小权限 Token 和真实写入授权后恢复；公开 CI 托管实跑与 Release Tag 也尚未形成外部证据。见 [M8 收口计划](docs/paichange-m8-resume-release-plan.md)与 [M8 实施记录](docs/paichange-m8-implementation.md)。Jira、Webhook、多 SCM 同时启用、真实 IdP、登录页/Refresh Token、SCIM/组织同步仍不属于该收口版本。M6a Docker 只形成有限的单机任务执行边界；M6b/M7a/M7b 也只形成生产存储、身份与运维的最小切片，不等于高可用、多区域、Kubernetes 或完整生产部署。
