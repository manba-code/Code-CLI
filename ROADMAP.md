...
- 运行时模型切换：新安装默认 `deepseek / DeepSeek-V4-pro`；`/model glm-5.1` / `/model glm-5v-turbo` 明确切 GLM 模型，`/model deepseek` / `/model step` / `/model kimi` 切 provider 并读取配置里的具体模型
...
**下一步**：真人总人时 9-session 可行性试跑已完成并通过流程审计，中文更正报告为 `docs/change-spec-human-effort-feasibility-20260823-01-zh.md`；机械流程审计通过，但测后说明最终 `ACCEPT` 未基于具体代码审阅，因此真人效率不可评价。契约锁定、范围治理、验证闭环和可审计性的工程价值有正面证据，量化收益尚未证明。正式研究前必须预注册人时非劣界值、补齐 A 路径可比产品耗时、强化 transcript/计时采集，并复盘公开 Verifier 未阻止 false acceptance 的原因。13 个现有 fixture 已覆盖显式非目标、兼容性、安全边界、状态转换、跨文件约束和统一澄清记录，并补齐公开证据下限与单点突变；Quick 历史失败也已归因修复。任务与证据变更发生在历史 Pilot 之后，后续真实结果必须建立新版基线；13×3×2 或 13×3×3 的新完整复跑仍未获授权。
...
后续开发计划 M1、M2、M3、M4、M5、M6a、M6b、M7a 与 M7b 最小切片已完成：见 [PaiChange 后续开发计划](docs/paichange-next-development-plan.md)。Draft 异步生成、Human Evidence、可信 Principal/RBAC 和项目工具策略已分别验收。M4 已接入单一 GitLab 的最小闭环：Issue IID 幂等导入、任务分支 push、MR 对账/创建和绑定当前 head 的 commit status，并用本地 HTTP 假 GitLab + bare remote 覆盖结果不明恢复；真实 GitLab 测试 project 仍待单独授权验收，详见 [M4 实施记录](docs/paichange-m4-implementation.md)。M6a 新增任务级 Docker 命令/Verifier 执行平面和控制面 Evidence 哈希归档，当前 Docker Desktop 目标主机验收已闭环。M6b 已增加 PostgreSQL V1 迁移、ChangePersistence、数据库租约队列、S3-compatible Evidence、PostgreSQL SCM ledger 和 SQLite 离线迁移，并在真实 PostgreSQL 17/MinIO 本地容器中通过迁移、重开、缓存重建、重复投递和 Worker 租约恢复闭环。M7a 将 schema 升到 V2，补齐单 issuer JWT/JWKS 验证、PostgreSQL 成员目录、乐观版本、成员审计与一次性 bootstrap，并通过本地假 OIDC/JWKS + PostgreSQL 容器闭环。M7b 增加健康/指标、生产启动校验、成员审计导出、日志脱敏和 PostgreSQL+S3 备份恢复验证；本地容器覆盖 JWKS/数据库/S3/SCM 故障、重启、租约与重复投递，从停写恢复点实测 RPO 2 秒、RTO 2 秒。详见 [M7a 实施记录](docs/paichange-m7a-implementation.md)与 [M7b 实施记录](docs/paichange-m7b-implementation.md)。生产镜像/代理/daemon 加固、真实 IdP、目标环境备份恢复、监控、容量与更广泛恢复仍是部署前工作。
...
M8 GitHub-first 发布收口已完成：已交付 GitHub Issue/branch/PR/status Adapter、`mock|gitlab|github` 单选装配、loopback 假 GitHub + bare remote 闭环和三类 GitHub Actions；Workflow、Worker、API、RBAC、Evidence 与 publication ledger 没有按 provider 分叉。真实 GitHub Issue #1、任务分支、PR #3、head-bound Status、响应不明后的重启对账、普通/容器 CI、`v16.1.1` Release、JAR/SHA-256 与无凭据启动 smoke 均已形成公开证据。现有 GitLab Adapter 与回归测试作为兼容能力保留，真实 GitLab 已从 M8 退出条件中剔除，如未来恢复须独立立项并授权。见 [M8 收口计划](docs/paichange-m8-resume-release-plan.md)与 [M8 实施记录](docs/paichange-m8-implementation.md)。Jira、Webhook、多 SCM 同时启用、真实 IdP、登录页/Refresh Token、SCIM/组织同步仍不属于该收口版本。M6a Docker 只形成有限的单机任务执行边界；M6b/M7a/M7b 也只形成生产存储、身份与运维的最小切片，不等于高可用、多区域、Kubernetes 或完整生产部署。
