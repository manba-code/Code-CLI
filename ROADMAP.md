...
后续开发计划 M1、M2、M3、M4、M5、M6a、M6b 与 M7a 最小切片已完成：见 [PaiChange 后续开发计划](docs/paichange-next-development-plan.md)。Draft 异步生成、Human Evidence、可信 Principal/RBAC 和项目工具策略已分别验收。M4 已接入单一 GitLab 的最小闭环：Issue IID 幂等导入、任务分支 push、MR 对账/创建和绑定当前 head 的 commit status，并用本地 HTTP 假 GitLab + bare remote 覆盖结果不明恢复；真实 GitLab 测试 project 仍待单独授权验收，详见 [M4 实施记录](docs/paichange-m4-implementation.md)。M6a 新增任务级 Docker 命令/Verifier 执行平面和控制面 Evidence 哈希归档，当前 Docker Desktop 目标主机验收已闭环。M6b 已增加 PostgreSQL V1 迁移、ChangePersistence、数据库租约队列、S3-compatible Evidence、PostgreSQL SCM ledger 和 SQLite 离线迁移，并在真实 PostgreSQL 17/MinIO 本地容器中通过迁移、重开、缓存重建、重复投递和 Worker 租约恢复闭环。M7a 将 schema 升到 V2，补齐单 issuer JWT/JWKS 验证、PostgreSQL 成员目录、乐观版本、成员审计与一次性 bootstrap，并通过本地假 OIDC/JWKS + PostgreSQL 容器闭环；详见 [M7a 实施记录](docs/paichange-m7a-implementation.md)。生产镜像/代理/daemon 加固、真实 IdP、目标环境备份恢复、监控、容量与更广泛恢复仍是部署前工作。
...
GitLab Adapter 已交付但真实实例验收已暂缓：当前无法提供专用测试 project，待后续单独提供测试 project、Issue、最小权限 Token 和真实写入授权后恢复；这不属于 M4 代码验收失败。Jira/GitHub、Webhook、多 SCM、真实 IdP 验收、登录页/Refresh Token、SCIM/组织同步，以及 M6b 的生产备份恢复、监控和容量验收仍未交付。M6a Docker 只形成有限的单机任务执行边界；M6b/M7a 也只形成最小生产存储与身份切片，不等于高可用、多区域、Kubernetes 或完整生产部署。启动、验收和边界见 README 的 PaiChange 部分及 M3–M7a 实施记录。
